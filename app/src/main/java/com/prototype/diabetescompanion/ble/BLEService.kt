...
//            BLEConnectionManager.writeCharToStartReceivingBGLValues(clearValue)
        ...
        override fun onCharacteristicChanged(
            ...
            Util.makeLog("onCharacteristicChanged()")
    ...
    fun connect(address: String?): Boolean {
        try {
            ...
            mBluetoothGatt = device.connectGatt(this,
                false,
                mGattCallback)                //Directly connect to the device so autoConnect is false
    ...
    fun setCharacteristicIndication(characteristic: BluetoothGattCharacteristic, enabled: Boolean) {
        try {
            ...
            mBluetoothGatt!!.setCharacteristicNotification(characteristic,
                enabled)//Enable notification and indication for the characteristic
            ...
            for (des in characteristic.descriptors) {
                if (null != des) {
                    ...
                    Log.i(TAG,
                        "***********************SET CHARACTERISTIC INDICATION SUCCESS**")//Write the descriptor
                }
            }
        } catch (e: Exception) {
            e.message?.let { Log.i(TAG, it) }
    ...
    fun writeCharacteristic(characteristic: BluetoothGattCharacteristic) {
        Util.makeLog("writeCharacteristic()")
        try {
            ...
            if (mBluetoothGatt!!.writeCharacteristic(characteristic)) {                       //Request the BluetoothGatt to do the Write
                Log.i(TAG,
                    "****************WRITE CHARACTERISTIC SUCCESSFUL**$characteristic")//The request was accepted, this does not mean the write completed
                /*  if(characteristic.getUuid().toString().equalsIgnoreCase(getString(R.string.char_uuid_missed_connection))){

                }*/
            } else {
                Log.i(TAG,
                    "writeCharacteristic failed")                                   //Write request was not accepted by the BluetoothGatt
            }
    ...
    fun readCharacteristic(characteristic: BluetoothGattCharacteristic) {
        ...
        Log.i(TAG, "READING CHAR: ${characteristic.uuid.toString()}")
        val status =
    ...
    fun setCharacteristicNotification(
        ...
        try {
            ...
            for (des in characteristic.descriptors) {
                if (null != des && des.uuid.toString().contains("2902")) {
                    Util.makeLog("Descriptor found with UUID: ${des.uuid.toString()}")
                    des.value =
